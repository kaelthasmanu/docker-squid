...
class squidstat{
	...
	function enableDebug($level=1){
		$this->debug = true;
		$this->debug_level = (int)$level;
		$this->debug_log = array();
	}

	function logDebug($msg, $level=1){
		if ($this->debug && $level <= $this->debug_level){
			$this->debug_log[] = date('c')." [L$level] ".(string)$msg;
		}
	}
	...
	function writeDebugToFile($path){
	...
	*/
	...
	function makeQuery($pass=""){
		$raw = array();
		$this->logDebug('Starting makeQuery', 1);
		// sending request
		if(!$this->fp){
			$this->logDebug('File pointer is not set - no connection', 1);
		...
		if($pass!=""){
			...
			$this->logDebug('Added Authorization header', 2);
		}
		$out .= "\r\n";
		$this->logDebug('Writing request to socket', 2);
		$written = @fwrite($this->fp, $out);
		if ($written === false) {
			$this->logDebug('Failed to write to socket', 1);
			$this->errno = 2;
			$this->errstr = 'Failed to write request to socket';
			return false;
		}
		// read response with a simple timeout guard
		...
		while (!feof($this->fp)) {
			...
			if ($line === false) {
				...
				if ($meta['timed_out']){
					$this->logDebug('Socket read timed out', 1);
					$this->errno = 3;
					$this->errstr = 'Timed out while reading response from server';
					fclose($this->fp);
					return false;
				}
				break;
		...
		fclose($this->fp);
		$this->logDebug('Read '.count($raw).' lines from socket', 2);
		if (count($raw) == 0){
			$this->logDebug('Empty response from server', 1);
			$this->errno = 4;
			$this->errstr = 'Empty response from server';
			return false;
		}
		// Accept both HTTP/1.0 and HTTP/1.1 successful responses
		if (!preg_match('#^HTTP/\d\.\d\s+200\s+OK#', $raw[0])){
			$this->logDebug('Unexpected HTTP status: '. $raw[0], 1);
			$this->errno=1;
